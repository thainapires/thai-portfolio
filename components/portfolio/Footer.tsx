...
export function Footer() {
    ...
    return (
        <footer className="portfolio-container grid gap-6 border-t border-border py-8 text-text-secondary md:grid-cols-3 md:items-center lg:min-h-36 lg:py-0">
            ...
            {/* Default */}
            <nav className="link-hover-primary flex items-center justify-center gap-x-4 text-lg font-[850] sm:hidden" aria-label={dictionary.footer.socialLabel}>
            ...
            <p className="m-0 text-center text-sm sm:text-start sm:text-base md:text-center">
                © {new Date().getFullYear()} Thainá. {dictionary.footer.copyright}
            </p>

            {/* sm+ */}
            <nav className="link-hover-primary hidden flex-wrap items-center gap-x-4 gap-y-3 text-lg font-[850] sm:flex md:justify-self-end" aria-label={dictionary.footer.socialLabel}>
                {socialLinks.map((link) => (
                    <FooterSocialIcons key={link.label} socialLink={link} />
                ))}
