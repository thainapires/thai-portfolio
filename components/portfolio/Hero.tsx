...
function HeroTerminalArrow() {
    ...
    if (shouldReduceMotion) {
        return (
            <svg
                aria-hidden="true"
                className="pointer-events-none absolute left-0 top-0 z-20 hidden w-36 -translate-x-2/3 -translate-y-2/3 -rotate-3 text-text-primary dark:text-[#818496] md:block lg:w-52 lg:-translate-x-3/4"
                viewBox="0 0 176 112"
                fill="none"
            >
                <path
    ...
    return (
        <motion.svg
            aria-hidden="true"
            className="pointer-events-none absolute left-0 -top-1 z-0 hidden w-36 -translate-x-2/3 -translate-y-2/3 -rotate-3 text-text-primary dark:text-[#818496] md:block lg:w-52 lg:-translate-x-3/4"
            viewBox="0 0 176 112"
            fill="none"
        >
            <motion.g
                ...
                <div className="reveal-on-load relative z-10 mx-auto w-full max-w-md min-w-0 text-center sm:max-w-lg md:mx-0 md:text-left md:justify-self-end lg:max-w-md xl:max-w-lg">
                    <p className="mb-2 text-lg font-medium sm:mb-3 sm:text-2xl">
                        <span className="relative inline-block">
                            <img
                                aria-hidden="true"
                                className="dark:hidden pointer-events-none absolute left-1/2 top-1/2 z-0 w-[calc(100%+2rem)] max-w-none -translate-x-1/2 -translate-y-1/2 -rotate-2 select-none sm:w-[calc(100%+2.5rem)]"
                        ...
                        <div className="overflow-hidden rounded-xl border border-white/10 dark:border-white/[0.08] bg-[#211f27] dark:bg-[#1E2233]">
                            <div className="flex items-center justify-between border-b border-white/10 dark:border-white/[0.08] bg-[#2c2934] dark:bg-[#292E43] px-3 py-2 sm:px-4 sm:py-3">
                                <div className="flex items-center gap-2" aria-hidden="true">
                                    ...
                                    <span className="size-3 rounded-full bg-primary dark:bg-[#C084FC]" />
                                </div>

                                <span className="hidden font-mono text-xs font-bold text-white/60 dark:text-[#A9ACBB] sm:block">
                                    ~/thainapires/home
                                </span>
                            </div>

                            <div className="space-y-4 p-3 font-mono text-xs leading-relaxed text-white dark:text-[#F1F2F7] sm:space-y-6 sm:p-6 sm:text-base">
                                <div className="space-y-3 sm:space-y-4">
                                    {terminalLines.map(({ prompt, command, output, extra }) => (
                                        <div key={command}>
                                            <p className="break-words">
                                                <span className="text-primary dark:text-[#C084FC]">
                                                    {prompt}
                                                </span>

                                                <span className="text-white/45 dark:text-[#818496]">
                                                    :${' '}
                                                </span>

                                                <span className="text-accent-yellow dark:text-[#F4D06F]">
                                                    {command}
                                                </span>
                                            </p>

                                            <p className="mt-1 max-w-sm text-white/70 dark:text-[#B8BAC8]">
                                                {output}
                                            </p>
                                ...
                                <div className="flex flex-wrap items-center gap-2 border-t border-white/10 dark:border-white/[0.08] pt-3 text-xs font-bold uppercase text-white/50 dark:text-[#A9ACBB] sm:pt-4">
                                    <span className="rounded-full bg-white/10 dark:bg-white/[0.07] px-3 py-1.5">
                                        React
                                    </span>
                                    ...
                                    <span className="inline-flex items-center gap-2 text-primary dark:text-[#C084FC]">
                                        <span className="size-2 animate-pulse rounded-full bg-primary dark:bg-[#C084FC]" />
                                        {t.ready}
                                    </span>
                                </div>
                            </div>
